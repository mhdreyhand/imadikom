...
<div class="container mx-auto px-4 mt-6 mb-6">
    <h1 class="text-4xl font-bold mb-2 text-center font-sans">Daftar Alumni IMADIKOM</h1>

    @auth
    <!-- Display the "Tambah Alumni" button only for authenticated users -->
    <a href="{{ route('alumni.create') }}" class="bg-sky-400 hover:bg-sky-500 text-white font-bold py-2 px-4 rounded mb-4 inline-block font-sans">Tambah Alumni</a>
    @if(session('success'))
    ...
    <!-- Formulir Pencarian -->
    <form method="GET" action="{{ route('alumni.index') }}" class="mb-4" id="filterForm">
        <input type="text" name="search" placeholder="Cari Alumni..." class="border focus:outline-none focus:ring focus:border-sky-300 px-4 py-2 rounded font-sans" value="{{ request('search') }}">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap font-sans">
                        <a href="{{ route('alumni.show', $a->id) }}" class="text-black hover:text-sky-500">
                            {{ $a->nama }}
                        </a>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap font-sans">{{ $a->angkatan }}</td>
                    @auth
                    <!-- Display the "Edit" and "Hapus" buttons only for authenticated users -->
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="{{ route('alumni.edit', $a->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-2 font-sans">Edit</a>
                        <form action="{{ route('alumni.destroy', $a->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-900 font-sans" onclick="return confirm('Apakah Anda yakin ingin menghapus alumni ini?')">Hapus</button>
                        </form>
                    </td>
                    @endauth
                </tr>
